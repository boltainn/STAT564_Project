...
author: "Önder TÜRE - tureonder@gmail.com"
format:
  html:
    code-fold: true
editor: visual
...
In this project, data is 4 different tennis singles 2013 grand slams match statistics. In the data, there are different statistics about match like foul, number of firs serve win, and others that will be explained in the next part. Aim of the project is predict the result of the tennis match by using different match statistics. Another aim is to find most effective match statistics for the win.

## 2. Source of the data variables, dependent variable

```{r combine_DF}
## Getting data
AusOpenMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/AusOpen-men-2013.csv")
AusOpenWoMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/AusOpen-women-2013.csv")

FrenchOpenMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/FrenchOpen-men-2013.csv")
FrenchOpenWoMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/FrenchOpen-women-2013.csv")

USOpenMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/USOpen-men-2013.csv")
USOpenWoMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/USOpen-women-2013.csv")

WimbledonMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/Wimbledon-men-2013.csv")
WimbledonWoMen <- read.csv("https://raw.githubusercontent.com/boltainn/STAT564_Project/main/Wimbledon-women-2013.csv")
##
...
## 4. Exploratory Data Analysis(EDA) and Confirmatory Data Analysis(CDA)

### Summary Statistics

Below there are frequency tables for tournament, round, result and gender. Aus open and French open tournaments have most number of match. Also, as expected most matches played at round 1. Number of male matches are greater than female matches. Moreover, Most round 1 matches played at US open, but at the remaining round US Open matches are not much when considering other tournaments. There may be incomplete data for US Open. Number of result 1 and 2 are very close to each other. So, data is balanced.
...
Also, set result summary is like below, as seen, there are a lot of NA values for ST4 and ST5 especially. This is because most of the match is not last to 5 or 4 sets. It is over in 2 or 3 sets.
...
On this part, ACE variable is created by summing of ACE.1 and ACE.2. Similarly other variables are summing and total of player 1 and player 2 statistic variables are created. Aim of this research is investigating effect of different tournament on player performance. Each tournament have different surface type so this should be effect the match statistics. As seen in the box plots, at French open tournament, players do less ace than other tournaments. Clay surface (French open surface) can lead to this, because clay surface slow down the ball. Also, in the box plot of DBF, we can say that players do more double foul at the US and AUS open tournaments. Both tournaments has hard surfaces. Additionally, there is significant difference on male and female statistics for ACE, male can do more ACE than women according to this graph but number of sets are not same for male and female matches. Female matches are best of 3, but male matches are best of 5. So, this can be reason for that.
...
As seen on bubble plot, when BPC increase, TPW increase because when player break the opponent player serve, match can be extend. Also number of first serve win tend to increase when TPW and BPC increase.
...
```{r first}
## to check variances after cleaning, before data saved to another name.
GrandSlam_before <- GrandSlam
```
...
## This is saved to V2 for alternative dataset to conduct TPW regression
GrandSlam_V2 <-GrandSlam
###
GrandSlam_Reg <- GrandSlam_V2%>%
  filter(!is.na(TPW.1))
...
After, manipulating and imputating data set, variances of variables are not change much as you see in the bar graph. (Y scale is log.)

```{r var_check, warning=FALSE}
## Variances change check after imputation. All stats are divided by TG for before case to compare final and before imputation case.
GrandSlam_before$TG <- rowSums(GrandSlam_before[, c("ST1.1", "ST2.1", "ST3.1", "ST4.1","ST5.1",
                                       "ST1.2", "ST2.2", "ST3.2", "ST4.2","ST5.2")], na.rm = TRUE)

GrandSlam_before <- GrandSlam_before %>%
  mutate(FSW.1 = as.numeric(FSW.1/TG), FSW.2 = as.numeric(FSW.2/TG), SSW.1 = as.numeric(SSW.1/TG),
         SSW.2=as.numeric(SSW.2/TG),ACE.1 = as.numeric(ACE.1/TG), ACE.2 = as.numeric(ACE.2/TG),
         DBF.1=as.numeric(DBF.1/TG),DBF.2=as.numeric(DBF.2/TG),WNR.1=as.numeric(WNR.1/TG),
         WNR.2=as.numeric(WNR.2/TG),UFE.1=as.numeric(UFE.1/TG),BPC.1=as.numeric(BPC.1/TG),
...
variances_first <- GrandSlam_before %>%
  filter(across(everything(), ~ !is.infinite(.))) %>%
  summarise(across(where(is.numeric), \(x) var(x,use = "complete.obs")))

variances_last <- GrandSlam %>%
  summarise(across(where(is.numeric), \(x) var(x,na.rm = TRUE)))

variances_first <- variances_first %>%
  select(c(colnames(variances_last)))

var_comp <- rbind(variances_first,variances_last)
var_names <- colnames(var_comp)
var_comp$group <- c("Before Imputation", "After Imputation")

df_long <- var_comp %>%
  pivot_longer(cols =var_names , names_to = "variable", values_to = "value")

ggplot(df_long, aes(x = variable, y = value, fill = group)) +
  geom_bar(stat = "identity", position = position_dodge(width = 1)) +
  labs(title = "Comparison of Variables Across Multiple Variables",
       x = "Variable",
       y = "Variance",
       fill = "Group") +
  scale_y_log10()+
...
## 6. Classification Statistical Modelling

### Train-Test Data Set Preparation

GrandSlam data set splited by 20% test and 80% training by random selection. Before modelling, numeric variables in the train and test data set should be scaled. Firstly, train set is scaled and then, by using train set mean and standard deviation test set should be scaled too. Test and train sets are scaled after splitting, because, test observations should not be effect anything on the train data set in order to maintain integrity of model evaluation. After scale the train data set summary is like below.

```{r train_test, warning=FALSE}
#creating test training data by considering distribution of Result
set.seed(25)
...
SVM is used to predict the RESULT of the tennis matches on this project. As a cross validation method, k-fold cv is used for k equals 10. This cv is made by using "caret" package in r. SVM linear applied because of creating interpretable model.

```{r SVM_tuning, warning=FALSE}
## SVM tunning process
...
Cost parameter tuned as `r best_C` after 10 k fold cv by minimizing error. SVM model remake with best cost value. Importance of variables can be interpreted from coefficients, because data set is scaled. According to coefficients, FSW, BPC, UFE and SSW are most important variables to win the match.
...
kable(coefs[1,order1], col.names = "Coefficient", caption = "Ordered Cofficients")
```

Confusion matrix of test data:

```{r SVM_performance, warning=FALSE}
# Extract the final predictions from the cross-validation
...
SVM has a `r performance_table_svm$Value[1]` accuracy for test data. Important variables are like logistic regression.

### Random Forest Model

Random forest model mtry parameter is tuned with 10 k fold cross validation for 2 to 28 grid. 750 number of tree used for modeling to reach minimum error.

```{r random_f}
# Train the model using random forest with tuning
rf_model <- train(RESULT ~ ., data = GrandSlam_trn_std, method = "rf", trControl = train_control, tuneLength = 28,ntree=750)
...
After tuning process, mtry tuned as `r best_mtree` considering accuracy. Also, OOB error stabilize after 600-700 trees. Therefore 750 trees is acceptable for modeling.

```{r rf_conf}
rf_last <- randomForest(x=GrandSlam_trn_std[,-20], y=GrandSlam_trn_std$RESULT,ntree=1000, mtry = best_mtree)
Test_pred <- predict(rf_last, newdata = GrandSlam_tst_std)
...
Neural network with one hidden layer that has 1 node is used to predict Result. There is one hidden layer with one node because, we want to make more interpretable model. Before appliying ANN, all factor variables change to numerical to conduct ANN.
...
Accuracy of test data is `r conf_matrix$overall['Accuracy']`.

As seen on the schema, output weights and biases are very close to each other. Therefore, variable weights can be related to importance of variable. Weight table shows that FSW, BPC and SSW are the most effective variables to predict result.
...
Model comparison table show that Random Forest model has the best accuracy. Also when we consider importance of variable, all of the model indicate that FSW, BPC, SSW are the most effective statistics that effect the result. Therefore, players should practice serving better and returning serves from opponent in order to win the match.
...
kable(comp_df, caption = "Performance Comparison")

```

## 7. Regression Modelling for TPW

In regression part, we separate the player statistics and create new data set like below. Tournament, round and gender parameters are removed because their effect on result is not much according to classification modeling results. Also, Result column removed because our aim is find most effective player statistics to create more points in regression part.

```{r TPW_data_prepare, warning=FALSE}
GrandSlam_Reg <- GrandSlam_V2%>%
  filter(!is.na(TPW.1))
GrandSlam_Reg <- GrandSlam_Reg%>%
  select(-c(FNL.1,FNL.2,RESULT))
...
kable(head(GrandSlam_Reg_final), caption = "Head Of Regression Data Set")

kable(summary(GrandSlam_Reg_final),caption = "Summary of Regression Data Set")
```
...
ACE, DBF, UFE, WNR and NPA have small effect on TPW according to parameters. Also UFE is not significant parameter to predict TPW. On the other hand FSW, SSW and BPC has significant and positive effect on TPW. Also, R square is 0.87 that is acceptable.
...
Ridge regression can be used to find effective parameters and predict TPW for players. Grid search for lambda shows that FSW, WNR and BPC are the most effective parameters to win more points like in Lasso.
...
plot(ridge.mod,main="L2 norm vs Coefficients")
...
RMSE comparison show that LASSO is the best model for predicting TPW.
...
perf_lm_lasso_ridge <- data.frame("Metric"= "RMSE","Linear Model"=perf_lm$X.Value.....rmse_lm,"LASSO"=perf_lasso$X.Value.....RMSE_lasso,"RIDGE"=perf_ridge$X.Value.....RMSE_ridge)
kable(perf_lm_lasso_ridge,col.names = c("Metric", "Linear Model","LASSO","RIDGE"))
```

## Conclusion

As a conclusion, Grand slams 2013 data set analyzed by considering Result and total points win. For predicting result, classification modeling applied as logistic regression, svm, random forest and ANN. Random forest show the best performance for predicting match result. Also, according to all classification model FSW and BPC are the most effective variables. Moreover, TPW regression made by classic linear regression, lasso and ridge. All of the regression model show that again FSW and BPC are most important variables for TPW. Therefore, players should focus on serving and returning opponent services to win the match and win more points.
